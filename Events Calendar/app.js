...
async function fetchEvents() {
  try {
    const response = await fetch(
      "https://c0544972-043a-421a-832e-cb5cb0882472-00-3ldpzyn2u0w4.sisko.replit.dev/events.php?action=get_events"
    );
    ...
    eventsData = [
      {
        ...
        category: "academic",
        location: "Room 101",
        description: "Algebra intro",
      },
      {
        id: 2,
        title: "Football",
        datetime: "2025-04-02T16:00",
        category: "sports",
        location: "Field 1",
...
function renderCalendar(events) {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const dayEvents = pageEvents.filter(
      (e) => new Date(e.datetime).getDate() === day
    );
    dayEvents.forEach((e) => {
...
function renderList(events) {
  ...
  events.forEach((e) => {
    ...
    row.innerHTML = `
      <td><a href="#event-detail" onclick="showEventDetail(${e.id})">${
      e.title
    }</a></td>
...
document.getElementById("eventForm").addEventListener("submit", async (e) => {
  ...
  let datetime = document.getElementById("eventDateTime").value.trim();
  ...
  if (id) {
    await fetch(
      "https://c0544972-043a-421a-832e-cb5cb0882472-00-3ldpzyn2u0w4.sisko.replit.dev/events.php?action=update_event",
      {
        method: "POST",
    ...
    await fetch(
      "https://c0544972-043a-421a-832e-cb5cb0882472-00-3ldpzyn2u0w4.sisko.replit.dev/events.php?action=create_event",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    console.log({ title, datetime, location, category, description });
...
async function deleteEvent() {
  ...
  if (confirm("Are you sure you want to delete this event?")) {
    await fetch(
      "https://c0544972-043a-421a-832e-cb5cb0882472-00-3ldpzyn2u0w4.sisko.replit.dev/events.php?action=delete_event",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
document.getElementById("commentForm").addEventListener("submit", async (e) => {
  ...
  await fetch(
    "https://c0544972-043a-421a-832e-cb5cb0882472-00-3ldpzyn2u0w4.sisko.replit.dev/events.php?action=add_comment",
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function renderComments(eventId) {
  const container = document.getElementById("commentsSection");
  container.innerHTML = "";
  try {
    const response = await fetch(
      `https://c0544972-043a-421a-832e-cb5cb0882472-00-3ldpzyn2u0w4.sisko.replit.dev/events.php?action=get_comments&event_id=${eventId}`
    );
